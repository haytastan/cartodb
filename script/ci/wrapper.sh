...
do
    # Get database owner
    owner=$(psql -h $dbServer -U $dbAdmin -t -c "select r.rolname from pg_database d, pg_roles r where d.datname='carto_db_test' and d.datdba = r.oid")
    newDatabase="${databaseName}_${i}";
    # Create the database with specific owner and template
    $(psql -h $dbServer -U $dbAdmin -t -c "create database $newDatabase with owner $owner template $databaseName;") >> parallel_tests/wrapper.log 2>&1
