...
# Drop all databases
databases=$(psql -h $dbServer -U postgres -t -c "select datname from pg_database where datname like 'carto_db_test_%'")
...
do
  [ -z "$line" ] && continue
  psql -h $dbServer  -U postgres -t -c "drop database $line" >> parallel_tests/cleaner.log
done < parallel_tests/databases_new.log


# Drop all user databases
databases=$(psql -h $dbServer  -U postgres -t -c "select datname from pg_database where datname like 'cartodb_test_user_%'")
...
databases=$(psql -h $dbServer  -U postgres -t -c "select datname from pg_database where datname like 'cartodb_user_%'")
touch parallel_tests/user_databases.log
echo $databases >> parallel_tests/user_databases.log
touch parallel_tests/user_databases_new.log
sed -e 's/\s\+/\n/g' parallel_tests/user_databases.log > parallel_tests/user_databases_new.log
